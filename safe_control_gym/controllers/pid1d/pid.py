...
class PID(BaseController):
    ...
    def run(self,
        ...
        for i in range(iterations):
            ...
            cur_pos=np.array([obs[0], 0, 0])
            cur_quat=np.array(p.getQuaternionFromEuler([0, 0, 0]))
            cur_vel=np.array([obs[1], 0, 0])
            ...
            if self.env.TASK == Task.TRAJ_TRACKING:
                ...
                target_pos=np.array([self.reference[0], 0, 0 ])
                target_vel=np.array([self.reference[1], 0, 0 ])
            else:
